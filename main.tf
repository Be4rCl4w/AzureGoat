...
provider "azurerm" {
  ...
  subscription_id = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" # <-- Add this
}
...
# Generate random text for unique resource names
...
# SAS Token for Storage Container
locals {
  now       = timestamp()
...
# Replace environment variables for backend function
...
# Upload Backend Function App to Storage
...
# Service Plan for Function Apps (replaces azurerm_app_service_plan)
resource "azurerm_service_plan" "app_service_plan" {
  name                = "appazgoat${random_id.randomId.dec}-service-plan"
  resource_group_name = var.resource_group
  location            = var.location
  os_type             = "Linux"
...
# Backend Linux Function App (replaces azurerm_function_app)
...
# Upload files to production container
...
resource "azurerm_storage_blob" "app_files_dev" {
  ...
  depends_on = [
    null_resource.file_replacement_upload,
    azurerm_storage_container.storage_container_dev
  ]
}

# Upload files to VM container
resource "azurerm_storage_blob" "app_files_vm" {
  ...
  depends_on = [
    null_resource.file_replacement_upload,
    azurerm_storage_container.storage_container_vm
  ]
}
...
# Linux Virtual Machine (replaces azurerm_virtual_machine)
...
SETTINGS
  ...
  depends_on = [
    null_resource.file_replacement_upload,
    azurerm_storage_blob.app_files_prod
  ]
}
...
# Upload Frontend Function App to Storage
...
# Upload updated config to containers
resource "azurerm_storage_blob" "config_update_prod" {
  name                   = "shared/files/.ssh/config.txt"
  storage_account_name   = azurerm_storage_account.storage_account.name
  storage_container_name = azurerm_storage_container.storage_container_prod.name
  type                   = "Block"
